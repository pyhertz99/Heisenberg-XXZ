...
parameters with dB = 0 and plots entanglement entropy
specified site as a function of J_z.
"""
...
entropy_means_plot_neel = np.full((len(delta_B_array),len(J_z_array)),0.0)
    ...
    for j in range(len(delta_B_array)):
        ...
        for sample in range(samples):
            psi_array = np.load(folder + "neel/N" + str(N) + "-Z" 
                                ...
                                + ".npy")
            entropy_array = entanglementEntropyArray(psi_array, t_steps, PI, site, N)
            means[sample] = np.mean(entropy_array)
            
        entropy_means_plot_neel[j,i] = np.mean(means)
...
for i in range(len(delta_B_array)):
    ...
    delta_B_string = f"{delta_B:.2f}"
    plt.plot(J_z_array, entropy_means_plot_neel[i], linestyle="-", marker="o", label="dB = " + delta_B_string)
