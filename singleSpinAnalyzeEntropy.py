...
parameters with dB = 0 and plots entanglement entropy of
...
entropy_means_plot_ss = np.full((len(delta_B_array),len(J_z_array)),0.0)
...
for i in range(len(J_z_array)):
    ...
    for j in range(len(delta_B_array)):
        ...
        entropy_means_plot_ss[j,i] = np.mean(means)
...
for i in range(len(delta_B_array)):
    ...
    delta_B_string = f"{delta_B:.2f}"
    plt.plot(J_z_array, entropy_means_plot_ss[i], linestyle="-", marker="o", label="dB = " + delta_B_string)
