...
survival_means_plot_dw = np.full((len(delta_B_array),len(J_z_array)),0.0)
...
for i in range(len(J_z_array)):
    ...
    for j in range(len(delta_B_array)):
        ...
        survival_means_plot_dw[j,i] = np.mean(means)
...
for i in range(len(delta_B_array)):
    ...
    delta_B_string = f"{delta_B:.2f}"
    plt.plot(J_z_array, survival_means_plot_dw[i], linestyle="-", marker="o", label=r'$\Delta B =$' + delta_B_string)
