...
survival_arrays_plot_dw = np.full((len_delta_B_array,t_steps),0.0)
J_z_string = f"{J_z:.2f}"
...
for j in range(len_delta_B_array):
    ...
    for i in range(t_steps):
        ...
        survival_arrays_plot_dw[j,i] = np.mean(t_slice)
...
fig, ax = plt.subplots(1,1, figsize=(1.5*6,6))

for i in range(len(delta_B_array)):
    delta_B = delta_B_array[i]
    delta_B_string = f"{delta_B:.2f}"
    plt.plot(ts, survival_arrays_plot_dw[i], linestyle="-", label=r'$\Delta B = $' + delta_B_string)
