"""
Creates samples of domain-wall state evolution for given
parameters and saves it as binary files. Discrete
finite range topology implemented.
"""
...
J_z_array = [1.0] #coupling constant J_z

B_0 = 0.0 #magnetic field mean value
delta_B_array = np.linspace(0.0,250.0,26) #magnetic field spread
...
folder = "./data/discFiniteRange/dB-analyze/K10/"

len_delta_B_array = len(delta_B_array)
len_J_z_array = len(J_z_array)

#specify how many samples to create
samples_array = np.full((len_delta_B_array,len_J_z_array),SAMPLES)
...
for i in range(len(J_z_array)):
    ...
    for j in range(len(delta_B_array)):
        ...
        for sample in range(samples):
            ...
            np.save(folder + "domain-wall/N" + str(N) + "-Z" 
